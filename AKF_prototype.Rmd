...
kalman_smoother <- function(
  ...
  while(sqrt(sum(((old_proc_scale - post_proc_scale) / nrow(data))^2) +
        sum(((old_meas_scale - post_meas_scale) / nrow(data))^2)) > 10^(-12)){
    print(
...
kalman_filter_minibatch <- function(
  ...
  proc_cov_mats <- lapply(1:1000,
                          function(x) riwish(v = prior_proc_df, S = prior_proc_scale))
  meas_cov_mats <- lapply(1:1000,
                          function(x) riwish(v = prior_meas_df, S = prior_meas_scale))
  psis_k <- 0
  iter_count <- 1
  data_count <- 0
  parallel_states <- lapply(1:1000, function(x) initial_state)
  innovation_covar_mats <- lapply(1:1000, function(x) innovation_covar)
  post_cov_mats <- lapply(1:1000, function(x) post_covar)
  ...
  while(psis_k < 0.7){
    ...
    psis_weights <- sort(exp(colSums(log(psis_weights))))[801:1000]
