...
kalman_smoother <- function(
  ...
  while(sqrt(sum(((old_proc_scale - post_proc_scale) / nrow(data))^2) +
    ...
    print(
      sqrt(sum(((old_proc_scale - post_proc_scale) / nrow(data))^2) +
...
check_smoother <- kalman_smoother(
  ...
  prior_state = c(2.7, -2.2, 2.5),
  meas_mat = trans_mat,
  prior_meas_df = 100,
  prior_meas_scale = diag(0.01^2, nrow = ncol(trans_mat)),
  prior_proc_df = 100,
  prior_proc_scale = diag(0.001^2, nrow = nrow(trans_mat)),
...
kalman_filter_minibatch <- function(
  ...
  print('before the while loop in minibatch')
  ...
  while(psis_k < 0.7){
    ...
    temp_states <- mapply(
      function(initial_state, meas_covar, proc_covar, post_covar){
        ...
        return(list(
          latent_states = as.numeric(temp_file[[2]][,1:nrow(meas_mat)]),
    ...
    print('before computing importance weights')
    
    importance_weights[[iter_count]] <- as.numeric(unlist(mapply(
      function(innovation_covar, state) dmvnorm(
        data[iter_count+1, 2:4], # need to fix this
        mean = trans_mat %*% state,
...
#    psis_weights <- sort(exp(colSums(log(psis_weights))))[161:200]
    psis_weights <- exp(sort(colSums(psis_weights))[161:200])
    print(psis_weights)
...
adaptive_kalman_filter <- function(
  ...
  if(burn_in > 0){
    ...
    output_states[[minibatch_count]] <- smoother[[2]]
    output_meas[[minibatch_count]] <- smoother[[3]]
    ...
    prior_meas_df <- prior_meas_df / batch_total_time + smoother[[4]]
    prior_meas_scale <- prior_meas_scale / batch_total_time + smoother[[5]]
    
    prior_proc_df <- prior_proc_df / batch_total_time + smoother[[6]]
    prior_proc_scale <- prior_proc_scale / batch_total_time + smoother[[7]]
    
    prior_covar <- smoother[[8]]
    innovation_covar <- smoother[[9]]
  }
  
  print('got past initial update')
  ...
  while(itercount < nrow(data)){
    ...
    smoother <- kalman_smoother(
      data = data[(itercount+1):(itercount+batch_size),],
      prior_state = prior_state,
      meas_mat = meas_mat,
      post_covar = prior_covar,
      prior_meas_df = prior_meas_df,
    ...
    output_states[[minibatch_count]] <- smoother[[2]]
    output_meas[[mininbatch_count]] <- smoother[[3]]
  ...
  output_states_combined$timestamp <- shift(data$timestamp, n = 1, type = 'lead')
  
  output_meas_combined <- rbindlist(output_meas)
  output_meas_combined$timestamp <- shift(data$timestamp, n = 1, type = 'lead')
  
  return(list(
    input_data = data,
    states = output_states_combined,
    predicted_meas = output_meas_combines,
    post_meas_df = prior_meas_df,
    post_meas_scale = prior_meas_scale,
    post_proc_df = post_proc_df,
    psot_proc_scale = post_proc_scale,
    post_covar = prior_covar,
    innovation_covar = innocation_covar
  ))
}
```
...
dat <- fread('./dataset/toyset/processed_toyset.csv')
dat$timestamp <- as.POSIXct(dat$timestamp)
dat <- dat[,lapply(.SD, function(x) x * 100), by = .(timestamp, time_gap)]

check_smoother_init <- kalman_smoother(
  data = dat[19000:19999],
  meas_mat = trans_mat, 
  prior_state = rep(0, 3), 
...
check_akf <- adaptive_kalman_filter(
  data = dat[20000:25000], 
  meas_mat = trans_mat, 
  burn_in = 1000,
  prior_state = rep(0, 3), 
  prior_covar = diag(10, 3), 
  innovation_covar = diag(0.1, 3),
  prior_proc_df = 10, 
  prior_proc_scale = diag(10 * 0.1, 3),
  prior_meas_df = 10, 
  prior_meas_scale = diag(10 * 0.1, 3)
